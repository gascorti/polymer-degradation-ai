...
def build_curve_level_dataset(df_long: pd.DataFrame, metadata_cols: list = None) -> pd.DataFrame:
    """
    ...
    `metadata_cols` son las columnas de propiedades a preservar (un valor por
    curva, el primero del grupo); por defecto usa el esquema sintético. El
    esquema real (ver src/data/schema.py) pasa su propia lista, más rica.
    """
    if metadata_cols is None:
        metadata_cols = [
            "polymer_type", "medium", "temperature_C", "pH",
            "initial_mw_kDa", "crystallinity_pct", "surface_area_mm2",
        ]
    ...
    for curve_id, group in df_long.groupby("curve_id"):
        ...
        record["assay_duration_days"] = group["time_days"].max()
        record["n_points"] = len(group)
        record["k_day_inv"] = k
        record["kinetic_fit_r2"] = r2
